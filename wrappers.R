...
ep_dds <- function(fn, p_bounds, t_iter= 1000, n_cores= 4,r= 0.2, ...) {
  ...
  p_bounds$initial=apply(data.frame(p_bounds$min,p_bounds$max), 1,
  ...
  while (i < t_iter) {
    ...
    if(t_iter==5000){
      if(i < round(2/5*t_iter,-2)){p_iter= 1000
      } else if(i <round(7/10*t_iter,-2)){p_iter= 500
      } else if(i <round(9/10*t_iter,-2)){p_iter= 100
      } else {p_iter= 10
      }
    }
    else{
      if(i < round(2/5*t_iter,-2)){p_iter= 1000
      } else if(i <round(1/2*t_iter,-2)){p_iter= 500
      } else if(i <round(4/5*t_iter,-2)){p_iter= 100
      } else {p_iter= 10
      }
    }
    
    ep_dds_results = clusterCall(cl = my_cluster,
                                 dds,
                                 fn= model_wrapper,
