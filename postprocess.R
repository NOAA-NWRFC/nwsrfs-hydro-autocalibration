...
import::from(dplyr, filter, select, summarise, group_by, ungroup, mutate, arrange, rename,bind_rows,
...
import::from(rfchydromodels,sac_snow_uh,sac_snow,sac_snow_states,lagk,sac_snow_uh_lagk,lagk,
...
for(basin in basins){
  ...
  for(results_dir in results_dirs){
    ...
    zones = optimal_pars$zone |> unique() |> as.character() |> sort() |> str_subset('-')
    ...
    }else{obs_inst = obs_daily_q95 = NULL}
    ...
    }else{optimal_routed_raw = optimal_routed = optimal_routed_states =  NULL}
    ...
    if(cu){
      ...
      optimal_sim_6hr_inst = optimal_cu_daily |> as_tibble() |>
    ...
    if(cv){
      ...
      valid_sim_daily = semi_join(as_tibble(optimal_sim_daily), as_tibble(forcing_valid_fold_daily),
                                  by=c("year", "month", "day")) |> as.data.table()
      ...
      if(n_zones>0){
        ...
        optimal_states_6hr = anti_join(optimal_states_6hr, forcing_valid_fold,
                                       by=c("year", "month", "day", "hour")) |> as.data.table()
      }else{valid_states_6hr = optimal_states_6h= NULL}
      ...
      if(n_upstream != 0){
        ...
        optimal_routed_states = anti_join(optimal_routed_states, forcing_valid_fold,
                                      by=c("year", "month", "day","hour")) |> as.data.table()
      }else{valid_routed = valid_lagk_states = NULL}
    ...
    output_plots <- function(x,plot_path){
      ...
      cyclical_daily = data.frame(x)|>
        group_by(month,day)|>
        summarise(sim_flow_cfs=mean(sim_flow_cfs),
                  flow_cfs=mean(flow_cfs),
                  .groups='drop')|>
        mutate(wyear = ifelse(month>=10,1999,2000),
